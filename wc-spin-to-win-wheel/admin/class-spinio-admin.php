...
class spinio_Admin
{
    ...
    */
    public function export_spinio_subscribers()
    {
        ...
        $results = $wpdb->get_results("SELECT `name` ,  `email` , `status`  FROM {$tablename};", ARRAY_A);
        $csv_fields = array();
        $csv_fields[] = 'Name';
        $csv_fields[] = 'Email';
        $csv_fields[] = 'Status';
    ...
    foreach ($results as $Result) {
      ...
      //  $status = mb_convert_encoding($Result['status'], 'UTF-8', 'ISO-8859-1'); 
      //$status = mb_convert_encoding($Result['status'], 'Windows-1252', 'UTF-8');
     // $status = mb_convert_encoding($Result['status'], 'ISO-8859-1', 'UTF-8');
     // $status = utf8_encode($Result['status']);
      // $status = utf8_decode($Result['status']);
     // $status = iconv("UTF-8", "ISO-8859-1//TRANSLIT", $Result['status']);
      $status = iconv("UTF-8", "Windows-1252//TRANSLIT", $Result['status']);
     // $status = mb_convert_encoding($Result['status'], 'UTF-8', 'auto');


        // Thêm dòng vào tệp CSV với dữ liệu đã chuyển đổi
        $leadArray = array($Result['name'], $Result['email'], $status);
