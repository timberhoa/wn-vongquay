...
/*
 * Các chức năng công cộng của plugin.
 *
 * Xác định tên plugin, phiên bản và hai hooks ví dụ cho cách
 * nạp stylesheet và JavaScript công cộng.
 *
 * @package    spinio
...
class spinio_Public
{
     ...
     * @since    1.0.0
     *
     * @var string Phiên bản hiện tại của plugin này
     */
    private $version;

    /**
     * Khởi tạo lớp và thiết lập các thuộc tính của nó.
     *
     * @since    1.0.0
     *
     * @param string $plugin_name Tên của plugin
     * @param string $version     Phiên bản của plugin này
     * @param      string    spinio_path    Đường dẫn hằng số của plugin này
     */
    ...
    /**
     * Đăng ký các stylesheet cho phía công cộng của trang web.
     *
     * @since    1.0.0
     */
    public function enqueue_styles()
    {
     ...
     /**
     * Đăng ký các JavaScript cho phía công cộng của trang web.
     *
     * @since    1.0.0
     */
    ...
    /**
     *   Hàm để lấy tất cả các lát từ bảng tùy chọn.
     */
     ...
     * Hàm để lấy chuỗi JSON của bánh xe
     * Hàm này lấy các tùy chọn lát và cài đặt hiển thị để tạo chuỗi JSON.
     */
    ...
    /**
     * Hàm để lấy tất cả các người đăng ký
     */
    public function spinio_set_subscriber()
    {
        $spinio_msg = null;
        $spinio_data = $_POST;
        if (isset($_POST['email']) && filter_var($_POST['email'], FILTER_VALIDATE_EMAIL)) { //kiêmr tra email hợp lệ không
            //email valid not lets check if it's already exits
            $spinio_data['email'] = sanitize_email($_POST['email']); // check email tồn tại chưa
            global $wpdb;
            ...
            if (empty($result)) {  // nếu email chưa tồn tại
                if (!isset($spinio_data['name'])) {
                    $spinio_data['name'] = null; // nếu name ko có thiết lập thành null
                }
                $wpdb->insert($table_name, array(  //Thêm dữ liệu mới vào cơ sở dữ liệu 
                    'name' => $spinio_data['name'],
                    'email' => $spinio_data['email'],
                    'date' => current_time('mysql', 1),
                    'status' => $spinio_data['status'],
                ));
                $spinio_msg['error'] = false;
            } else {
                $spinio_msg['error'] = true;
                $spinio_msg['email'] = 'Email đã được sử dụng, vui lòng nhập lại';
            }
        } else {
            $spinio_msg['error'] = true;
            $spinio_msg['email'] = 'Email không hợp lệ hoặc trống, vui lòng nhập lại';
        }
        echo json_encode($spinio_msg);// chuyển đổi mảng thông điệp phản hồi thành dạng JSON
        die();
    }

    /**
     *Hàm để hiển thị bánh xe. Phụ thuộc vào hàm is_show.
     */
    ...
    /**
     *  xác định liệu vòng quay nên được hiển thị hay không dựa trên các cookie và cài đặt hiển thị.
     */
     public function is_show()
{
   
    $post_content = get_post_field('post_content', get_the_ID());
    if (strpos($post_content, '[wn-vong-quay]') !== false) {
